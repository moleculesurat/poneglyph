/* Intake by folder — upload every PDF in an inbox tree against its kind, then
   ...
   when known); on 201 it then POSTs /api/documents/<id>/read unless --no-read. */
...
for (const ent of readdirSync(inbox, { withFileTypes: true }).sort(byName)) {
  ...
  } else if (isPdf(ent.name)) {
    plan.push({ path: abs, rel: ent.name, kindId: null }); // volunteered
  } else {
    skipped.push(`${ent.name}  (not a PDF — skipped)`);
  }
}
...
for (const p of plan) {
  ...
  const nAsks = (data.document.requirementIds ?? []).length;

  if (noRead) {
    console.log(`${p.rel}  ${id}  ${kindCol}  ${nAsks} asks  (no read)`);
    continue;
  }
  ...
  try {
    ...
    readsFailed += 1;
    console.log(`${p.rel}  ${id}  ${kindCol}  ${nAsks} asks  read network-error: ${e.message || "fetch failed"}`);
    continue;
  }
  const rdata = await jsonOf(rres);
  if (!rres.ok) {
    readsFailed += 1;
    console.log(`${p.rel}  ${id}  ${kindCol}  ${nAsks} asks  read ${rres.status}: ${rdata.error ?? ""}`);
    continue;
  }
  readsOk += 1;
  ...
  console.log(`${p.rel}  ${id}  ${kindCol}  ${nAsks} asks  satisfies ${t.satisfies} · partial ${t.partial} · no ${t.no} · ${nFields} fields`);
}

console.log(
  `summary: ${plan.length} PDF(s) seen · ${uploaded} uploaded · ${already} already stored · ` +
    `${readsOk} read ok · ${readsFailed} read failed · ${volunteered} volunteered · ${skippedNonPdf} skipped non-PDF`,
);
